...
from .agent import AgentToolbox, build_agent_plan, is_summary_goal
from .agent_graph import extract_goal_timestamps_ms
from .retrieval import plan_evidence_requirements
    ...
    def add_segment(item: dict[str, Any], requirement_id: str) -> None:
        ...
        if key not in by_segment and len(by_segment) < MAX_LEDGER_EVIDENCE:
            ...
            by_segment[key] = {
                ...
                "dbEvidenceId": int(item.get("evidenceId", item.get("evidence_id", item.get("id", 0))) or 0),
...
def _resolved_citations(draft: dict[str, Any], ledger: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for evidence_id in draft.get("evidenceIds", [])[:MAX_REPORT_EVIDENCE]:
        ...
        citations.append({
            ...
            "dbEvidenceId": int(item.get("dbEvidenceId", item.get("evidence_id", 0)) or 0),
...
def _route_after_critic(state: StructuredAgentState) -> str:
    ...
    if int(state.get("revision_count", 0)) < 1:
        return "retrieve_followup_evidence"
    return END


def _retrieve_followup_node(toolbox: AgentToolbox, state: StructuredAgentState) -> dict[str, Any]:
    """Re-run slot retrieval once after Critic identifies a weak draft."""
    result = _retrieve_node(toolbox, state)
    ledger = result.get("evidence_ledger") or {}
    statistics = dict(ledger.get("statistics") or {})
    statistics["followupRetrieval"] = True
    ledger["statistics"] = statistics
    return {"evidence_ledger": ledger}
...
def run_structured_evidence_agent(
    ...
    workflow.add_node(
        ...
        lambda state: _retrieve_followup_node(toolbox, state),
    )
    ...
    workflow.add_edge("retrieve_followup_evidence", "verify_followup_evidence")
    workflow.add_edge("verify_followup_evidence", "revise_report")
